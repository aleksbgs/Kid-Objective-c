...
-(void)loadButtons
{
	...
	if ([arrCategories count]==1) {
		backgroundImageView.image = [UIImage thumbnailImage:@"alphabet_main_bg_ipad.jpg"];
		UIButton *btnCategory = [arrButtons objectAtIndex:0];
		btnCategory.frame = CGRectMake(20, 10, 520, 150);		
		@try {
		...
		backgroundImageView.image = [UIImage thumbnailImage:@"full_main_bg_ipad.jpg"];
	}
...
-(IBAction)play:(id)sender
{
	...
	@try {
		...
		UIViewController *viewController = [dicViewControllers objectForKey:[NSString stringWithFormat:@"%@ViewController_iPad",[category.EnglishTitle capitalizedString]]];
		if (!viewController) {
			viewController =  [[NSClassFromString([NSString stringWithFormat:@"%@ViewController_iPad",[category.EnglishTitle capitalizedString]]) alloc] initWithNibName:[NSString stringWithFormat:@"%@View_iPad",[category.EnglishTitle capitalizedString]] bundle:nil];
			[dicViewControllers setObject:viewController forKey:[NSString stringWithFormat:@"%@ViewController_iPad",[category.EnglishTitle capitalizedString]]];
		}
		[viewController setCategory:category];
		[self.navigationController pushViewController:viewController animated:YES];
		if (![viewController isKindOfClass:[NSClassFromString(@"AlphabetsViewController_iPad") class]]) {
			[dicViewControllers removeObjectForKey:[NSString stringWithFormat:@"%@ViewController_iPad",[category.EnglishTitle capitalizedString]]];
...
{
	SettingsViewController_iPad	*settingsViewController = [[SettingsViewController_iPad alloc]initWithNibName:@"SettingsView_iPad" bundle:nil];

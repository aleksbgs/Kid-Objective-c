...
#import "NumbersViewController_iPad.h"
@implementation NumbersViewController_iPad
...
-(void)dynamicallyLoadGraphicsForCountDown
{
	...
	for (int i=0; i<noOfBalls; i++) {
		...
		[self performSelector:@selector(fadeOut:) withObject:btnBall afterDelay:(noOfBalls-i-1)*COUNT_DOWN_ANIMATION_DURATION];
	}
}
-(void)dynamicallyLoadGraphicsForCountUp
{
	...
	for (int i=0; i<countOfBalls; i++) {
		...
		[arrBallViews addObject:btnBall];
		startPointX = startPointX + width + GRAPHIC_HORIZONTAL_SPACE;
		[self performSelector:@selector(fadeIn:) withObject:btnBall afterDelay:(i*COUNT_DOWN_ANIMATION_DURATION)+1];

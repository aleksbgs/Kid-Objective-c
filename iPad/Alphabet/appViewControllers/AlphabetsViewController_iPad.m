//
//  AlphabetsViewController_iPad.m
//  PreSchool
//
...
#import "AlphabetsViewController_iPad.h"

@implementation AlphabetsViewController_iPad
-(void)setCategory:(Category *)category{
	...
	//[btnCharCenter setTitle:[arrObjects objectAtIndex:0] forState:UIControlStateNormal] ;
}
...
-(void)setFrameOfBunny
{
	...
	if (frame.origin.x>-viewBunny.frame.size.width && frame.origin.x<(self.view.frame.size.width+viewBunny.frame.size.width)) {
		//NSLog([NSString stringWithFormat:@"Bunny frame:%.2f,%.2f",viewBunny.frame.origin.x, viewBunny.frame.size.width]);
	}
	else {
...
-(void)setFrameOfPaperPlane
{
	...
	if (frame.origin.x>-viewPaperPlane.frame.size.width && frame.origin.x<(self.view.frame.size.width+viewPaperPlane.frame.size.width)) {
		//NSLog([NSString stringWithFormat:@"Paper plane frame:%.2f,%.2f",viewPaperPlane.frame.origin.x, viewPaperPlane.frame.size.width]);
	}
	else {
...
-(void)setFrameOfSpider
{
	...
	if (frame.origin.x>-viewSpider.frame.size.width && frame.origin.x<(self.view.frame.size.width+viewSpider.frame.size.width)) {
		//NSLog([NSString stringWithFormat:@"SPider frame:%.2f,%.2f",viewSpider.frame.origin.x, viewSpider.frame.size.width]);
	}
	else {
...
-(void)moveSpider
{
	if (bViewAppeared) {
		if (!isAnimationOnScreen) {
			...
			if (![strOSVersion isEqualToString:@"3.2"]) {
	...
	if (bViewAppeared) {
		if (!isAnimationOnScreen) {
			...
			if (viewBird.tag==0) {
				...
				birdEndXPosition = (self.view.frame.size.width+viewBird.frame.size.width+70);
			}
		}
	...
	if (bViewAppeared) {
		if (!isAnimationOnScreen) {
			...
			if (viewPaperPlane.tag==0) {
				...
				paperPlaneEndXPosition = (self.view.frame.size.width+viewPaperPlane.frame.size.width+70);
			}
		}
		...
		if (!isAnimationOnScreen) {
			...
			for (int i=0; i<5; i++) {
				...
				CGPathAddCurveToPoint(thePath,
									  NULL,
									  point1.x,point1.y-150.0f,
									  point2.x,point1.y-150.0f,
									  point2.x,point1.y);
			}
...
- (void)audioPlayerDidFinishPlaying:(AVAudioPlayer *)avPlayer successfully:(BOOL)flag
{
	...
	else if (avPlayer==graphicPlayer) {
		[graphicPlayer release];
		graphicPlayer = nil;
	}
	else {
...
- (IBAction)graphic:(UIButton *)btnGraphic didStartDrag:(UIEvent *)event
{
	...
	if (btnGraphic==btnCloud) {
		...
		sunFrame.size.width = sunFrame.size.width+100;
		sunFrame.size.height = sunFrame.size.height+100;
		sunFrame.origin.x = sunFrame.origin.x-50;
...
- (void)didSwipe:(int)currentPage
{
	...
	frame.origin.x = 140 + (swipeView.frame.size.width * page);
	frame.origin.y = (swipeView.frame.size.height - btnCharCenter.frame.size.height)/2 - 18;
...
- (void)viewDidLoad {
	darkLayerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 768, 1024)];
...
- (void)didReceiveMemoryWarning {
	...
	[UIImage clearThumbnailCache];
	NSLog(@"did receive memory warnings…");

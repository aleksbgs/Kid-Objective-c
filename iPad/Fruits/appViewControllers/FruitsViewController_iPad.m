...
#import "FruitsViewController_iPad.h"
#import "Objects.h"

@implementation FruitsViewController_iPad
...
-(void)setBagOnOriginalPosition{
	NSLog(@"set frame");
	[[bagView layer] removeAllAnimations];
	btnBag.frame = CGRectMake(btnBag.frame.origin.x, btnBag.frame.origin.y, 100, 140);
	bagView.frame = btnBag.frame;
	bagBackImage.frame = CGRectMake(0, 0, btnBag.frame.size.width, btnBag.frame.size.height);
	bagFrontImage.frame = CGRectMake(0, 0, btnBag.frame.size.width, btnBag.frame.size.height);
...
-(void)setFrameOfBag{
	NSLog(@"set frame");
	[[bagView layer] removeAllAnimations];
	btnBag.frame = CGRectMake(bagView.frame.origin.x, bagView.frame.origin.y, 100, 140);
...
-(void)setFrameOfRoof{
	NSLog(@"setFrameOfRoof");
	CGRect frame = [[[btnRoof layer] presentationLayer] frame];
	CGFloat xPositon = frame.origin.x+frame.size.width/2;
	CGFloat yPosition = frame.origin.y+frame.size.height/2;
	btnRoof.center = CGPointMake(xPositon, yPosition);
}
...
-(void)setFrameOfView{
	...
	CGFloat slowFirstPostion = endXpostion+768;
	CGFloat slowSecondPositon = endXpostion +100;
	if (currentXPostionOfView < slowFirstPostion && currentXPostionOfView > slowSecondPositon) {
		moveViewDuration = VIEW_MOVE_DURATION/1.5;
		if (timerView) {
			[timerView invalidate];
			timerView = nil;
		}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView1{
	...
	if (scrollView1.contentOffset.x>8800) {
		setScrollFlag = YES;
		scrollView1.bounces = NO;
		[scrollView1 setContentSize:CGSizeMake(3300, 0)];
		//scrollView1.contentOffset = CGPointMake(scrollView1.contentOffset.x+50, 0);
		[scrollView1 setContentOffset:CGPointMake(scrollView.contentOffset.x+150, 0) animated:YES];
		...
		scrollView1.bounces = NO;
		[scrollView1 setContentOffset:CGPointMake(6400, 0)];
		[scrollView1 setContentOffset:CGPointMake(scrollView.contentOffset.x-300, 0) animated:YES];
	}
	else {
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView1{
	if(!scrollAnimation){
		...
		scrollView.frame = CGRectMake(0, 1024-FRUIT_BTN_WIDTH, 768, FRUIT_BTN_WIDTH);
...
-(void)loadFruitImages{
	...
	scrollView.frame = CGRectMake(0, 1024-FRUIT_BTN_WIDTH, scrollSize, FRUIT_BTN_HEIGHT);
	...
	btnBag.frame = CGRectMake(640, 784, 100, 140);
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag
{
	...
	if ([[anim valueForKey:@"roofAnimation"] isEqualToString:@"roof"]) {
		...
		btnRoof.frame = CGRectMake(0, -900, 768, 1080);
...
- (void)roof:(UIButton *)roof didStartDrag:(UIEvent *)event
{
	...
	if (touch.phase == UITouchPhaseMoved) {
		...
		CGFloat yAxis = center.y-900;
		if (yAxis<=0) {
			btnRoof.frame = CGRectMake(0, yAxis, 768, 1080);
		}
	}
}
...
-(void)swipeFruiet:(UIButton*)fruiet didStartDrag:(UIEvent*)event{
	...
	if (dragCount == 0) {
		...
		btnBigFruiet.frame = CGRectMake(fruiet.center.x, 1024-FRUIT_BTN_HEIGHT, 200, 200);
...
-(void)PopOutAllFruit:(id)sender{
	...
	btnBigFruiet.frame = CGRectMake(btn.center.x, 1024-FRUIT_BTN_HEIGHT, 200, 200);
	[btnBigFruiet addTarget:self action:@selector(fruit:didStartDrag:) forControlEvents:UIControlEventTouchDragInside | UIControlEventTouchDragOutside];
	[btnBigFruiet addTarget:self action:@selector(fruit:didEndDrag:) forControlEvents:UIControlEventTouchDragExit|UIControlEventTouchCancel|UIControlEventTouchUpOutside | UIControlEventTouchUpInside];
	...
	CGFloat shrinkValue =100/bagView.frame.size.width;
	[CATransaction begin];
...
-(void)showFruitInMainThread:(id)sender{
	...
	btnBigFruiet.frame = CGRectMake(btn.center.x, 1024-FRUIT_BTN_WIDTH, 200, 200);
	[btnBigFruiet addTarget:self action:@selector(fruit:didStartDrag:) forControlEvents:UIControlEventTouchDragInside | UIControlEventTouchDragOutside];
	[btnBigFruiet addTarget:self action:@selector(fruit:didEndDrag:) forControlEvents:UIControlEventTouchDragExit|UIControlEventTouchCancel|UIControlEventTouchUpOutside | UIControlEventTouchUpInside];
...
-(void)showFruitInNewThread:(id)sender{
	[self performSelector:@selector(showFruitInMainThread:) withObject:sender afterDelay:0];
}
-(void)clickOnSmallFruit:(id)sender {
	...
	if (dragCount==0) {
		...
		[self performSelectorOnMainThread:@selector(showFruitInNewThread:) withObject:sender waitUntilDone:NO];
...
- (void)viewWillAppear:(BOOL)animated{
	...
	[appDelegate setImage:[UIImage thumbnailImage:@"fruits-roof-ipad.png"] ofButton:btnRoof];
...
-(void)roofAnimated{
	...
	//positionAnimation.autoreverses = YES;
	CGMutablePathRef positonPath= CGPathCreateMutable();
	CGPathMoveToPoint(positonPath, NULL, [btnRoof layer].position.x, 540);
	CGPathAddLineToPoint(positonPath, NULL, [btnRoof layer].position.x, -360);
	positionAnimation.path = positonPath;
	positionAnimation.fillMode = kCAFillModeForwards;

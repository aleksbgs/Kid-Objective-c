...
#import "AnimalsViewController_iPhone.h"
//#define P(x,y) CGPointMake(x, y)

@implementation AnimalsViewController_iPhone
...
-(void)setFrameOfDragonFly:(NSTimer*)timerDragon{
	...
	NSLog(@"viewDragpn");
	if (frame.origin.x<-50||frame.origin.y<-18) {
		NSLog(@"remove timer");
...
-(void)putAnimalOnScreenInMainThread:(NSDictionary*)btnDictionary{
	...
	NSLog(@"direction random:- X %f and Y is :- %f and random X is:- %f and y is:- %f",directionRandomX,directionRandomY,randomX,randomY);
	...
	for (int i=0; i<kNoOfJumps; i++) {
		...
		NSLog(@"x position of point 1 :- %f and y positionof point 1 :-%f and x position of point 2 :-%f and y position of point 2:- %f",point1.x,point1.y,point2.x,point2.y);
		...
		else if(point1.x>250){
			point1.x = 280;
		}
		if (point2.x<20) {
			point2.x = 45;
		}
		else if(point2.x>250){
			point2.x = 280;
		}
		if (point1.x>250&&point1.y<150) {
			point1.x = 280;
			point1.y=180;
		}
		if (point2.x>250&&point1.y<150) {
			point2.x = 280;
			point1.y=150;
		}
...
-(void)addDragonFliesToScreen{
	NSLog(@"addDragonFiles");
	//[self playSoundEffect:@"dragonfly.mp3" withRepeatCount:0];
	if (!flagChestClose) {
		//for (int i=0; i<[arrDragonFliesOnScreen count]; i++) {
//			AnimateObjectView *imageView = [arrDragonFliesOnScreen objectAtIndex:i];
...
//		}
		...
		for (int i=0; i<randomCount; i++) {
			...
			int y = btnChest.center.y-40;//arc4random()% (int)self.view.frame.size.height;
			...
			CAKeyframeAnimation *positionAnimation = [CAKeyframeAnimation animationWithKeyPath:@"position"];
			NSLog(@"x position of dragon is:- %i and imageview y position is:- %f",randomX*direction,imageView.center.y);
			CGMutablePathRef positionPath = CGPathCreateMutable();
			CGPathMoveToPoint(positionPath, NULL, imageView.center.x ,  imageView.center.y );
			CGPathAddLineToPoint(positionPath, NULL,randomX*direction , -20);
			positionAnimation.path = positionPath;
...
-(void)playVOAudioPlayer:(NSTimer *)timer{
	NSLog(@"playVOAudioPlayer");
	NSString *soundFileName = [[timer userInfo] objectForKey:@"SoundName"];
	NSObject *object = [[timer userInfo] objectForKey:@"Object"];
	int repeatCount = [[[timer userInfo] objectForKey:@"RepeatCount"]intValue];
	@try {
		NSString *soundPath = [[NSString alloc] initWithFormat:@"%@/%@", resourcePath,soundFileName];
		//NSLog(@"Path to play: %@", resourcePath);
		...
		if( err ){
			...
			//[soundEffectsPlayer prepareToPlay];
			[soundEffectsPlayer play];
            NSString *position = [NSString stringWithFormat:@"%d",[arrObjects indexOfObject:object]];
			NSArray *array = [NSArray arrayWithObjects:soundEffectsPlayer,position,nil];
			[dicPlayers setObject:array forKey:[NSString stringWithFormat:@"VO#%@",[soundEffectsPlayer description]]];
		}
		[soundPath release];
	}
	@catch (NSException * e) {
...
-(void)slideAnimal:(UIButton *)btnAnimal
{
	...
	[self.view bringSubviewToFront:btnAnimal];
	CGMutablePathRef thePath = CGPathCreateMutable();
	CGPathMoveToPoint(thePath, NULL, 112.0f, 122.0f);
	...
	NSArray *controlPoints = [NSLocalizedString(@"iPhoneAnimalsSlideControlParameters",nil) componentsSeparatedByString:@";"];
	NSArray *points = [NSLocalizedString(@"iPhoneAnimalsSlideParameters",nil) componentsSeparatedByString:@";"];
	...
	btnAnimal.center = CGPointMake(264.00,220.00);
}
#pragma mark -
...
- (void)didClickFirstTime:(id)animView
{
	...
	if (obj.tag==100) {
		...
		//if (fishMoveDuration>=1.0f) {
		//		fishMoveDuration = FISH_MOVE_ANIMATION_DURATION/2.0f;
		//	}
		//[dicRemoveFish setObject:viewFish forKey:viewFish.ID];
...
-(void)transformAnimal:(UIButton *)btnAnimal
{
	...
	CAKeyframeAnimation *positionAnimation = [CAKeyframeAnimation animationWithKeyPath:@"position"];
	CGMutablePathRef positionPath = CGPathCreateMutable();
	CGPathMoveToPoint(positionPath, NULL, btnChest.center.x ,  btnChest.center.y-35 );
...
-(void)loadFishesForLeft
{
	...
	for (int i=0; i<MAX_FISHES ; i++) {
		...
		startYPosition = (self.view.frame.size.height-20);
		endYPosition=self.view.frame.size.height-150;
		NSLog(@"start x positon is:- %i and end x position is:- %i and start y postion is:- %i and end y position is:- %i",startXPosition,endXPosition,startYPosition,endYPosition);
		...
		NSLog(@"randon duration is:- %f",randomDuration);
